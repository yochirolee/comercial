...
async function renderExcelFirma(
  ...
  const firmaPath = getImagePath(empresa.firmaPresidente);
  if (firmaPath) {
    await addImageToExcel(workbook, worksheet, firmaPath, { col: 0.5, row: row - 1 }, { width: 100, height: 50 });
    row += 3;
  }
  ...
  const cunoPath = getImagePath(empresa.cunoEmpresa);
  if (cunoPath) {
    await addImageToExcel(workbook, worksheet, cunoPath, { col: 2, row: startRow + 1 }, { width: 70, height: 70 });
  }
...
export const ExportController = {
  ...
  async ofertaClienteExcel(req: Request, res: Response): Promise<void> {
    ...
    const cunoPath = getImagePath(empresa.cunoEmpresa);
    if (cunoPath) {
      await addImageToExcel(workbook, worksheet, cunoPath, { col: 2, row: firmaStartRow - 1 }, { width: 70, height: 70 });
    }
